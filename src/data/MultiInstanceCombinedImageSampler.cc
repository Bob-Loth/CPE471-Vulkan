#include "MultiInstanceCombinedImageSampler.h"
#include "vkutils/VmaHost.h"
#include <algorithm>
#include <cstring>
#include <string>

using instance_index_t = MultiInstanceCombinedImageSampler::instance_index_t;
...
MultiInstanceCombinedImageSampler::MultiInstanceCombinedImageSampler(
    ...
    mPaddedBlockSize(sLayoutSetAlignedSize(mBoundLayouts, mBufferAlignmentSize))
{
    ...
    if (!aDeviceBundle.isValid()) {
        throw std::runtime_error("MultiInstanceCombinedImageSampler may not be constructed with an invalid or partially valid device bundle!");
    }
...
void MultiInstanceCombinedImageSampler::setInstanceCount(instance_index_t aCount) {
    if (aCount > mCapacity) {
...
instance_index_t MultiInstanceCombinedImageSampler::pushBackInstance() {
    ++mInstanceCount;
...
instance_index_t MultiInstanceCombinedImageSampler::pushBackInstance(const UniformDataInterfaceSet& aDataInterfaces) {
...
UniformDataInterfaceSet MultiInstanceCombinedImageSampler::getInstanceDataInterfaces(instance_index_t aInstanceIndex) {
...
void MultiInstanceCombinedImageSampler::setInstanceDataInterfaces(instance_index_t aInstanceIndex, const UniformDataInterfaceSet& aDataInterfaces) {
    assertInstanceInbounds(aInstanceIndex);
    assertLayoutMatches(aDataInterfaces);
    std::for_each(aDataInterfaces.begin(), aDataInterfaces.end(), [](const auto& entry) {entry.second->flagAsDirty(); }); // Artificially flag as dirty since the new slot doesn't have this data.
...
void MultiInstanceCombinedImageSampler::freeInstanceDataInterfaces(instance_index_t aInstanceIndex) {
    const auto& finder = mBoundDataInterfaces.find(aInstanceIndex);
    if (finder != mBoundDataInterfaces.end()) mBoundDataInterfaces.erase(finder);
}

void MultiInstanceCombinedImageSampler::setCapcity(instance_index_t aCapacity) {
...
void MultiInstanceCombinedImageSampler::resizeToFit() {
...
bool MultiInstanceCombinedImageSampler::isBoundDataDirty() const {
...
void MultiInstanceCombinedImageSampler::pollBoundData() const {
    for (const std::pair<instance_index_t, UniformDataInterfaceSet>& mapEntry : mBoundDataInterfaces) {
        for (const std::pair<uint32_t, UniformDataInterfacePtr>& setEntry : mapEntry.second) {
...
void MultiInstanceCombinedImageSampler::updateDevice() {
...
size_t MultiInstanceCombinedImageSampler::getBoundDataOffset(uint32_t aBindPoint) const {
    return(mBoundLayouts.getBoundDataOffset(aBindPoint, mBufferAlignmentSize));
}

size_t MultiInstanceCombinedImageSampler::getBoundDataOffset(uint32_t aBindPoint, instance_index_t aInstanceIndex) const {
    assertInstanceInbounds(aInstanceIndex);
    return(mPaddedBlockSize * aInstanceIndex + getBoundDataOffset(aBindPoint));
}

const std::vector<VkDescriptorSetLayoutBinding>& MultiInstanceCombinedImageSampler::getDescriptorSetLayoutBindings() const {
    return(mLayoutBindings);
}

// TODO: Use flyweight or warn about cost of excessive use. 
std::map<uint32_t, VkDescriptorBufferInfo> MultiInstanceCombinedImageSampler::getDescriptorBufferInfos() const {
...
void MultiInstanceCombinedImageSampler::createDescriptorSetLayout() {
...
void MultiInstanceCombinedImageSampler::createBuffer(size_t aNewSize) {
    if (aNewSize == 0) {
...
void MultiInstanceCombinedImageSampler::autoGrowCapcity(instance_index_t aNewMinimumCapacity) {
...
void MultiInstanceCombinedImageSampler::resizeBuffer(size_t aNewSize) {
...
void MultiInstanceCombinedImageSampler::updateOffsets() {
...
void MultiInstanceCombinedImageSampler::updateSingleBinding(instance_index_t aInstance, uint32_t aBinding, const UniformDataInterfacePtr aInterface) {
    VmaAllocator allocator = VmaHost::getAllocator(mCurrentDevice);
...
void MultiInstanceCombinedImageSampler::setupDeviceUpload(VulkanDeviceHandlePair) {
    // Unused
}

void MultiInstanceCombinedImageSampler::uploadToDevice(VulkanDeviceHandlePair) {
    updateDevice();
}

void MultiInstanceCombinedImageSampler::finalizeDeviceUpload(VulkanDeviceHandlePair) {
    // Unused
}

void MultiInstanceCombinedImageSampler::assertInstanceInbounds(instance_index_t aIndex) const {
    if (aIndex >= mInstanceCount) throw InstanceBoundError(aIndex, mInstanceCount);
}

void MultiInstanceCombinedImageSampler::assertLayoutMatches(const UniformDataInterfaceSet& aDataInterfaces) const {
    for (const auto& layoutEntry : mBoundLayouts) {
...
void MultiInstanceCombinedImageSampler::_cleanup() {
    if (mUniformBuffer != VK_NULL_HANDLE) {
        VmaAllocator allocator = VmaHost::getAllocator(mCurrentDevice);
        vmaDestroyBuffer(allocator, mUniformBuffer, mBufferAllocation);
        mUniformBuffer = VK_NULL_HANDLE;
